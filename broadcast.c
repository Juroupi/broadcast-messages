...
int sendBroadcastMessage(BroadcastSocket s, const char* message) {
	return sendto(s->socket, message, strlen(message), 0, (struct sockaddr*)&s->sendAddr, sizeof(struct sockaddr_in));
}
